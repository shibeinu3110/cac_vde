...
from facepose import Facepose
from utils import eye_aspect_ratio, rec_to_roi_box, crop_img, draw_axis
from zeromq.SerializingContext import SerializingContext
...
# zmq for local publish
context = SerializingContext()
socket = context.socket(zmq.PUB)

# connect always to localhost for local
LOCAL_ZMQ_PORT = 5555

def main(userid):
    # Always connect to local host
    socket.connect(f"tcp://127.0.0.1:{LOCAL_ZMQ_PORT}")
    ...
    shape = None
    yaw_predicted = pitch_predicted = roll_predicted = 0, 0, 0
    record = None
    ...
    while cap.isOpened():
        ...
        time_elapsed = time.time() - prev
        if time_elapsed > 1.0 / frame_rate_use:
            prev = time.time()
            ...
            for rect in rects:
                ...
                leftEAR = eye_aspect_ratio(shape[36:42])
                rightEAR = eye_aspect_ratio(shape[42:48])
                ear = (leftEAR + rightEAR) / 2.0
                mar = mouth_aspect_ratio(shape[60:69])
                ...
                if yaw_predicted.item() < -30 or yaw_predicted.item() > 30:
                    ...
                    lostFocusDuration += time.time() - focusTimer
                    focusTimer = time.time()
                if -30 < yaw_predicted.item() < 30 and lostFocus:
                    lostFocusCount += 1
                    lostFocus = False
                    focusTimer = None
                ...
                # Publish locally via ZMQ
                data = {'id': str(userid), 'record': record}
                publish(frame, data)

        # Overlay counts and axes on display frame
        ...
        cv2.putText(frame_display, f"Lost Focus Duration: {round(lostFocusDuration)}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0), 1)
        cv2.putText(frame_display, f"Face Not Present Duration: {round(faceNotPresentDuration)}", (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0),1)

        if shape is not None:
            draw_border(frame_display, (rect.left(),rect.top()), (rect.left()+rect.width(), rect.top()+rect.height()), (255,255,255),1,10,20)
            draw_axis(frame_display, yaw_predicted.item(), pitch_predicted.item(), roll_predicted.item(), tdx=int(center_x), tdy=int(center_y), size=100)
            for idx, (x, y) in enumerate(shape):
                cv2.circle(frame_display, (x, y), 2, (255,255,0), -1)
...
def publish(image, data):
    if image.flags['C_CONTIGUOUS']:
        ...
        socket.send_array(img, data, copy=False)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Local attention monitor')
    parser.add_argument('--userid', help='user id', required=True)
    args = parser.parse_args()
